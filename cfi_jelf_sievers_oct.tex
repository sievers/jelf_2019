...
{\color{red} There are only a few BAO intensity mapping experiments
  ...
  with 512 dishes improving over CHIME by nearly a factor of 2 (and 
  1024 dishes by more than 3.5).  HIRAX is also
  located in the southern hemisphere (CHIME and Tianlai are both in
  the north) so CHIME and HIRAX data provide independent measurements 
  of dark energy that can be combined for improved precision.  HIRAX's southern
  location also lets it observe the Galactic center, which yields
  an advantage in searching for new pulsars.  The HIRAX site is
...
\citep{Recnik15} works at scale in the field as well, with
      {\color{red} CHIME having discovered 13 FRBs \citep{chime_frbs}
        --- NEED TO UPDATE THIS}.  Custom feeds have been designed and
      built, and we continue to iterate to improve their noise
      performance.
...
universe.  The maximum expected signal is set by simple physics, and so
...
antennas.  Chiang will lead site testing at promising locations we
will undertake to identify.

To carry out this work, Chiang will develop a new radio
instrumentation lab at McGill.  The lab will first and foremost have
the test equipment necessary to verify the performance and integration
of the \mbe.  It will also be used to build and test the test antenna
systems to be deployed in the Canadian north.  The lab will house the
sensitive benchtop RF test equipment (such as vector network
analyzers) needed for these roles, plus serve as home base to portable
RF test equipment that can verify system performance in the field.
...
telescope search for dark energy and FRBs, and will strengthen Canada's
leading role in the international cosmology community.
By number of input signals, the \mbe\ will be
the second-largest F-engine in the world, trailing only
CHIME\footnote{the total number of signals coming from the ASKAP, the
  Australian SKA Pathfinder, is also larger, but they are split into
  several independent groups which are not processed together}.  HIRAX
will begin construction late this year, with the 512-element stage
starting late 2020, and so it is on the same timescale as the \mbe.
Crucially, there will be an array of radio telescopes to feed the
\mbe.  While the full HIRAX array is not yet funded, we are actively
...
emphasized in the Canadian astronomy plan.  Recommendation~7 of the LRP
